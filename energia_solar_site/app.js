(() => {
      ...
      <table>
        ...
        <tbody>
          ${points.map((year) => {
            ...
            return `<tr>
              <td>${year} ano${year > 1 ? "s" : ""}</td>
              <td>${money(economy)}</td>
              <td>${money(economy - inv)}</td>
            </tr>`;
          }).join("")}
        </tbody>
      </table>
  ...
  function drawChart(inv, monthly, years) {
    ...
    if (points[points.length - 1][0] !== x(monthsTotal)) {
      points.push([x(monthsTotal), y(Math.min(monthly * monthsTotal, maxValue))]);
    }
    ...
    chart.innerHTML = `
      <svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Economia acumulada ao longo do tempo comparada ao investimento inicial">
        <line x1="${pad.left}" y1="${y(inv)}" x2="${width - pad.right}" y2="${y(inv)}" class="chart-invest"/>
        <path d="${path}" class="chart-line"/>
        <circle cx="${px}" cy="${py}" r="5" class="chart-point"/>
        <text x="${pad.left}" y="${height - 7}" class="chart-label">0</text>
        <text x="${width / 2}" y="${height - 7}" text-anchor="middle" class="chart-label">${Math.round(monthsTotal / 2)} meses</text>
        <text x="${width - pad.right}" y="${height - 7}" text-anchor="end" class="chart-label">${monthsTotal} meses</text>
        <text x="${pad.left + 5}" y="${Math.max(12, y(inv) - 7)}" class="chart-label">investimento</text>
        <text x="${Math.min(width - 5, px + 8)}" y="${Math.max(15, py - 9)}" class="chart-label">payback</text>
      </svg>
    `;
  }
  ...
  function calculateHouse() {
    ...
    $("#housePayback").textContent = `${payback.toLocaleString("pt-BR", {
      minimumFractionDigits: 1,
      maximumFractionDigits: 1
    })} meses`;

    $("#houseText").textContent =
      `neste cenário, o investimento seria recuperado em cerca de ${(payback / 12).toLocaleString("pt-BR", {
        minimumFractionDigits: 1,
        maximumFractionDigits: 1
      })} anos.`;

    /* Mantém a calculadora financeira e o cenário residencial conectados. */
  ...
  ["houseKwh", "houseBill", "houseInvestment", "houseSaving"].forEach((id) => {
    $(`#${id}`)?.addEventListener("input", calculateHouse);
  });
  ...
  function updateImpact(inv, monthlySave, years, kwh = Number($("#houseKwh")?.value || 400)) {
    ...
    const co2Factor = 0.24; // fator didático, não representa medição real.
    const co2 = annualEnergy * co2Factor;

    $("#co2Impact").textContent =
      `${Math.round(co2).toLocaleString("pt-BR")} kg`;

    $("#energyImpact").textContent =
      `${Math.round(annualEnergy).toLocaleString("pt-BR")} kWh`;
  }

  $("#calcBtn")?.addEventListener("click", calculate);
  ["investimento", "economia", "anos"].forEach((id) => {
  ...
  function selectAnswer(answer) {
    ...
    quizAnswers[currentQuestion] = {
      pergunta: q.pergunta,
      respostaSelecionada: answer,
      resposta: q.opcoes[answer],
      correta: q.correta,
      acertou: answer === q.correta
    };
    ...
    options.forEach((button) => {
      ...
      if (value === answer && value !== q.correta) button.classList.add("wrong");
    });
  ...
  async function saveQuizResult(){
    const name = ($( "#quizName" )?.value || "Visitante").trim().slice(0,60) || "Visitante";
    const payload = {
      nome: name,
      score,
      total: quizQuestions.length,
      percentual: quizQuestions.length ? Math.round((score / quizQuestions.length) * 100) : 0,
      respostas: quizAnswers,
      data: new Date().toISOString()
    };
    try{
      const response = await fetch("/api/quiz-results", {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});
      if(!response.ok) throw new Error("Falha ao salvar");
      let status = $("#quizSaveStatus");
      if(!status){ status=document.createElement("div"); status.id="quizSaveStatus"; status.className="quiz-save-status"; quiz.appendChild(status); }
      status.textContent="✓ Resultado registrado no projeto.";
    }catch(error){
      console.warn("Não foi possível registrar o quiz:", error);
      let status = $("#quizSaveStatus");
      if(!status){ status=document.createElement("div"); status.id="quizSaveStatus"; status.className="quiz-save-status"; quiz.appendChild(status); }
      status.textContent="Resultado exibido localmente. O servidor não está conectado.";
    }
  }
  ...
  quizBtn?.addEventListener("click", () => {
    if (!quizQuestions.length || quizBtn.textContent === "Jogar novamente") {
      startQuiz();
      return;
    }
    ...
    if (!answered) {
      ...
      if (selected) selectAnswer(Number(selected.dataset.answer));
    }
  });

  quizNext?.addEventListener("click", nextQuestion);

  /* Corrige seleção visual antes de responder pelo botão */
  quiz?.addEventListener("click", (event) => {
    const option = event.target.closest(".quiz-option");
    if (!option || answered) return;

    $$(".quiz-option").forEach((btn) => btn.classList.remove("selected"));
  ...
  $$(".decision-btn").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      $$(".decision-btn").forEach((btn) => btn.classList.remove("selected"));
      button.classList.add("selected");

      $("#decisionReason").hidden = false;
  ...
  $("#saveDecision")?.addEventListener("click", () => {
    ...
    if (!selectedDecision || !reason) {
      $("#decisionResult").textContent = "Escolha sua decisão e o principal motivo.";
      return;
    }

    const decisionText = selectedDecision === "sim"
      ? "Você decidiu investir."
      : "Você decidiu não investir neste cenário.";

    $("#decisionResult").textContent =
      `${decisionText} Principal motivo: ${reason}.`;
  });
