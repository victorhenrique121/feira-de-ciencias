...
async function migrateOldResults() {
  ...
  try {
    ...
    db.exec(`CREATE TABLE IF NOT EXISTS legacy_migrations (
    ...
    const insert = db.prepare(`
      INSERT INTO quiz_attempts (
        user_id, activity_id, score, total, correct_answers, wrong_answers,
...
app.get('/api/users/:id/history', (req, res) => {
  ...
  const history = db.prepare(`
    SELECT qa.id, a.slug AS activity_slug, a.name AS activity,
      qa.score, qa.total, qa.correct_answers, qa.wrong_answers,
      qa.percentage, qa.time_seconds, qa.created_at
    FROM quiz_attempts qa
    INNER JOIN activities a ON a.id = qa.activity_id
    WHERE qa.user_id = ?
    ORDER BY datetime(qa.created_at) DESC, qa.id DESC
  `).all(userId);
...
app.get('/api/users/:id/rewards', (req, res) => {
  ...
  const rewards = db.prepare(`
    SELECT r.id, r.slug, r.name, r.description, r.type, r.file_path, ur.unlocked_at
    FROM user_rewards ur
    INNER JOIN rewards r ON r.id = ur.reward_id
    WHERE ur.user_id = ?
    ORDER BY datetime(ur.unlocked_at) DESC
  `).all(userId);

  res.json(rewards);
});

/* Compatível com o front-end antigo: respostas detalhadas são aceitas, mas NÃO são armazenadas. */
